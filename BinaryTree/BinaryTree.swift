...
public class BinaryTree {
	...
	public func trimOutsidedataRange(min : Int, max : Int) {
		self.root = trimOutsidedataRange(curr : self.root, min : min, max : max)
	}
	
	private func trimOutsidedataRange(curr : Node?, min : Int, max : Int) -> Node? {
		guard let curr = curr else {
			return nil
		}
		curr.left = trimOutsidedataRange(curr : curr.left, min : min, max : max)
		curr.right = trimOutsidedataRange(curr : curr.right, min : min, max : max)
		if curr.value < min {
			return curr.right
		}
		if curr.value > max {
